...
test('renders table and headings in core view (alias)', async ({ page }) => {
...
test('can reset core view without errors (alias)', async ({ page }) => {
    await page.goto(fixtureUrl);
    await page.waitForFunction(() => Boolean((window as any).MarkBloomCoreTest));
...
test('renders common markdown layouts comprehensively (alias)', async ({ page }) => {
    await page.goto(fixtureUrl);
    await page.waitForFunction(() => Boolean((window as any).MarkBloomCoreTest));
