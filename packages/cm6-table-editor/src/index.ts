...
class TableWidget extends WidgetType {
  ...
  toDOM(): HTMLElement {
    ...
    wrapper.className = "cm-table-editor cm-table-editor-revogrid";
    ...
    const applyConfig = () => {
      ...
      grid.rowSize = defaultRowSize;
      grid.rowDefinitions = buildRowDefinitions(this.data);
      grid.tabIndex = 0;
      const fallbackHeight = estimateGridHeight(this.tableInfo.rowCount, this.data.header != null);
      applyGridHeight(grid, fallbackHeight);
...
function buildRowDefinitions(data: TableData): RowDefinition[] {
  return data.rows.map((row, index) => {
    const maxLines = Math.max(1, ...row.cells.map((cell) => countLines(cell.text)));
    return {
      type: "rgRow",
      index,
      size: defaultRowSize * maxLines,
    };
  });
}
...
async function applyGridHeightFromContent(
  ...
  try {
    ...
    applyGridHeight(grid, height);
  } catch {
    // ignore
  }
}

function applyGridHeight(grid: HTMLElement, height: number) {
...
export function tableEditor(options: TableEditorOptions = {}): Extension {
  ...
  const theme = EditorView.baseTheme({
    ...
    ".cm-content .cm-table-editor .rgCell": {
      whiteSpace: "pre-wrap",
    },
