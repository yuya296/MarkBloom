...
| Bridge                     | WebViewとホスト間の通信を抽象化する層。VS Code版はpostMessageでcoreにデータを渡す。                    | Host非依存でAPIを揃え、将来Electronでも再利用する。                       |
| Core                       | React + Markdownレンダリングの純粋UI層。テキストを受け取りread-only表示を担う。                        | WebViewやElectronから再利用可能。                                         |
